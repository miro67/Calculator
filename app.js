document.getElementById("hm").addEventListener('click', ()=>{
	document.getElementById("hid").innerHTML = `<div class="form-group">
	<h2 style="padding:3%;">المبلغ المراد تحويله : </h2>
	<input type="text" class="searchBox" placeholder="0.00">
</div>
<span>من</span>
<select class="from" id="sel1">
<option>اختيار من …</option>
					...
					<option value="ALL">ALL - Albanian Lek</option>
						...
						<div>
						<button class="btn convert" type="submit">تحويل</button>
						<!-- <button class="btn reset" onclick="clearVal()">Reset</button>-->
</div>
<h2>سعر الصرف : <span class="finalValue" style="color:green;"></span></h2>`
...
fromCurrecy.addEventListener('change', (event) => {resultFrom = `${event.target.value}`})
toCurrecy.addEventListener('change', (event) => {resultTo = `${event.target.value}`})
search.addEventListener('input', updateValue)
function updateValue(e){searchValue = e.target.value}
convert.addEventListener("click", getResults)
async function getResults() {
	const ras = await fetch("https://api.exchangerate-api.com/v4/latest/USD")
	ras.json().then(displayResults)
}
...
function clearVal() {
	...
	document.getElementsByClassName("finalValue").innerText = ""
}
...
document.getElementById("ha").addEventListener('click', ()=>{
	...
	<div class="bu">2</div>
            ...
            <div class="bu">0</div>
            <div class="bu">.</div>
            <div id="ay" class="bu">=</div>
			</div>
    </section>`
	const dasply = document.querySelector("#dsply");
	let buttons = Array.from(document.getElementsByClassName("nu"));
	buttons.map( button => {
		button.addEventListener('click',(e)=>{
			switch (e.target.innerText) {
			    ...
			    break;
			    case '←':
					dasply.innerText = dasply.innerText.slice(0,-1);
			    break;
			    case '=':
					if (dasply.innerText){
				    dasply.innerText = eval(dasply.innerText)
				}
				break;
				default:
					dasply.innerText += e.target.innerText
					break;
				}
			});
})
...
// 	<div id="ts"></div>
// <select id="from" id="sel1">
//     <option >من …</option>
//     <option value="1000">Meters</option>
//     <option value="3.2808">Feet</option>
//     <option value="cm">cm</option>
//     <option value="Yards">Yards</option>
//     <option value="Kilometers">Kilometers</option>
// </select>
// <select id="to" id="sel1">
//     <option >الى …</option>
//     <option value="1000">Meters</option>
//     <option value="3.2808">Feet</option>
//     <option value="cm">cm</option>
...
//     <option value="Kilometers">Kilometers</option>
// </select>
// 	<button id="ay">matr</button>`
// 	document.getElementById("ay").addEventListener("click",()=>{
// 		const num = document.getElementById("sear").value;
// 		const form = document.getElementById("from").addEventListener("click",(e)=>{
// 			e.target.value
// 			const feet = {
// 				m:[3.2808],
// 				in:[ft*12],
...
// 			}
// 		})
// 		const to = document.getElementById("to").addEventListener("change",(e)=>{
// 			e.target.value
// 		})
// 		// console.log(document.getElementById("ts").innerHTML = num*form)
// 		// const ariae = {
// 		// 	Feet:[3.2808]
// 		// }
// 		// document.getElementById("ts").innerHTML = (num/ariae.Feet[0])/ariae.Feet[0]
// 	})
// })